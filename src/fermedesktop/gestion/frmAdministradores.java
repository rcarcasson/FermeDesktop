...
public class frmAdministradores extends javax.swing.JInternalFrame {
    ...
    private void CargarDatos(){
        ...
        ControladorAdministrador ctrlAd = new ControladorAdministrador();
        ArrayList<Usuario> lista = ctrlAd.ObtenerAdministradores(txtCadena.getText());
        for (int i=0; i<lista.size();i++){
            modelo.addRow(new Object[]{lista.get(i).getId(),lista.get(i).getRut(),lista.get(i).getNombre(),
            lista.get(i).getDireccion(),lista.get(i).getTelefono(),lista.get(i).getMail(),lista.get(i).getCargo()});
        }
        jTablaAdministradores.setModel(modelo);
        jTablaAdministradores.getTableHeader().getColumnModel().getColumn(0).setMaxWidth(0);
        jTablaAdministradores.getTableHeader().getColumnModel().getColumn(0).setMinWidth(0);
        
        jTablaAdministradores.getColumn("ID").setMaxWidth(0);
        jTablaAdministradores.getColumn("ID").setMinWidth(0);
        jTablaAdministradores.getColumn("ID").setPreferredWidth(0);        
    }
    ...
    private String[] getColumnas(){
        String[] columnas = new String[]{"ID","RUT","Nombre","Dirección","Teléfono","Mail","Cargo"};
        return columnas;
    }
    ...
    private boolean ValidarCampos(){
        ...
        if (idAdministrador==0){
            if (txtRut.getText().length()==0 || txtNombre.getText().length()==0 || txtDireccion.getText().length()==0 ||
                    txtTelefono.getText().length()==0 || txtCorreo.getText().length()==0 || txtCargo.getText().length()==0 || cl1.length()==0 || cl2.length()==0){
            ...
            if (txtRut.getText().length()==0 || txtNombre.getText().length()==0 || txtDireccion.getText().length()==0 ||
                    txtTelefono.getText().length()==0 || txtCorreo.getText().length()==0 || txtCargo.getText().length()==0 ){
                JOptionPane.showMessageDialog(null, "Debe completar los datos del formulario para continuar.","Error",JOptionPane.ERROR_MESSAGE);
                return false;
            }else{
                if (clave1.length == clave2.length){
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Ingrese el rut o parte del nombre del administrador para realizar una búsqueda. El sistema buscará todos los registros coincidentes para desplegarlos.");
        ...
        jTablaAdministradores.getTableHeader().setReorderingAllowed(false);
        jTablaAdministradores.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jTablaAdministradoresMouseClicked(evt);
            }
        });
        jScrollPane1.setViewportView(jTablaAdministradores);
        jTablaAdministradores.getColumnModel().getSelectionModel().setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        // TODO add your handling code here:
        if (ValidarCampos()){
            int confirmar = JOptionPane.showConfirmDialog(null, "¿Desea guardar los datos del administrador?","Guardar",JOptionPane.YES_NO_OPTION);
            if (confirmar == JOptionPane.YES_NO_OPTION){
                Usuario u = new Usuario();
                u.setId(idAdministrador);              
                ...
                u.setTipo_usuario('A');
                if (idAdministrador == 0){
                    ControladorAdministrador ctrAd = new ControladorAdministrador();
                    if (ctrAd.AgregarAdministrador(u)){
                        Funciones f = new Funciones();
                        ...
                        if (ctrA.AgregarUsuario(a)){
                            JOptionPane.showMessageDialog(null, "Administrador generado correctamente.","Crear administrador",JOptionPane.INFORMATION_MESSAGE);
                            Limpiar();
                            CargarDatos();
                            CambiarEstado(false);
                        ...
                        JOptionPane.showMessageDialog(null, "Ocurrio un error al generar el administrador.","Crear empleado",JOptionPane.ERROR_MESSAGE);
                    }         
                }else{
                    ControladorAdministrador ctrAd = new ControladorAdministrador();
                    if (ctrAd.ModificarAdministrador(u)){
                        Funciones f = new Funciones();
                        ControladorAcceso ctrA = new ControladorAcceso();
                        Acceso a = new Acceso();
                        a.setUsuario(txtCorreo.getText());
                        ...
                        a.setUsuario_id(idAdministrador);
                        if (ctrA.ModificarUsuario(a)){
                            JOptionPane.showMessageDialog(null, "Administrador actualizado correctamente.","Modificar administrador",JOptionPane.INFORMATION_MESSAGE);
                            Limpiar();
                            CargarDatos();
                            CambiarEstado(false);
                            idAdministrador = 0;
                        ...
                        JOptionPane.showMessageDialog(null, "Ocurrio un error al modificar al administrador.","Error",JOptionPane.ERROR_MESSAGE);
                    }
                }
            }else{
                txtRut.requestFocus();
    ...
    private void jTablaAdministradoresMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTablaAdministradoresMouseClicked
        ...
        if (evt.getClickCount() == 2){
            int fila = jTablaAdministradores.getSelectedRow();
            if (fila != -1){
                DefaultTableModel modelo = (DefaultTableModel)jTablaAdministradores.getModel();
                idAdministrador = Integer.parseInt(modelo.getValueAt(fila, 0).toString());
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        // TODO add your handling code here:
        int confirmar = JOptionPane.showConfirmDialog(null, "¿Esta seguro de eliminar a este administrador?","Eliminar administrador",JOptionPane.YES_NO_OPTION);
        if (confirmar == JOptionPane.YES_NO_OPTION){
            ControladorAdministrador ctrAd = new ControladorAdministrador();
            ControladorAcceso ctrA = new ControladorAcceso();
            if (ctrA.EliminarUsuario(idAdministrador)){
                if (ctrAd.EliminarAdministrador(idAdministrador)){
                    JOptionPane.showMessageDialog(null, "Administrador eliminado satisfactoriamente del sistema.","Administrador eliminado",JOptionPane.INFORMATION_MESSAGE);
                    idAdministrador = 0;
                    Limpiar();
                    CambiarEstado(false);
                    btnNuevo.setEnabled(true);
                    ...
                    JOptionPane.showMessageDialog(null, "Ocurrio un error al eliminar al administrador del sistema.","Error",JOptionPane.ERROR_MESSAGE);
                }
            }else{
                JOptionPane.showMessageDialog(null, "Ocurrio un error al eliminar la cuenta de acceso del administrador.","Error",JOptionPane.ERROR_MESSAGE);
            }
        }
