...
public class frmProveedores extends javax.swing.JInternalFrame {
    ...
    private void CargarDatos(){
        ...
        ArrayList<Proveedor> lista = ctrlP.ObtenerProveedores(txtCadena.getText());
        for (int i=0; i<lista.size();i++){
            modelo.addRow(new Object[]{lista.get(i).getId(),lista.get(i).getRut(),lista.get(i).getNombre(),
            lista.get(i).getDireccion(),lista.get(i).getTelefono(),lista.get(i).getMail(),lista.get(i).getRubro()});
        }
        jTablaProveedores.setModel(modelo);
        jTablaProveedores.getTableHeader().getColumnModel().getColumn(0).setMaxWidth(0);
        jTablaProveedores.getTableHeader().getColumnModel().getColumn(0).setMinWidth(0);
        
        jTablaProveedores.getColumn("ID").setMaxWidth(0);
        jTablaProveedores.getColumn("ID").setMinWidth(0);
        jTablaProveedores.getColumn("ID").setPreferredWidth(0);        
    }
    ...
    private String[] getColumnas(){
        String[] columnas = new String[]{"ID","RUT","Nombre","Dirección","Teléfono","Mail","Rubro"};
        return columnas;
    }
    ...
    private boolean ValidarCampos(){
        ...
        if (idProveedor==0){
            if (txtRut.getText().length()==0 || txtNombre.getText().length()==0 || txtDireccion.getText().length()==0 ||
                    txtTelefono.getText().length()==0 || txtCorreo.getText().length()==0 || txtRubro.getText().length()==0 || cl1.length()==0 || cl2.length()==0){
            ...
            if (txtRut.getText().length()==0 || txtNombre.getText().length()==0 || txtDireccion.getText().length()==0 ||
                    txtTelefono.getText().length()==0 || txtCorreo.getText().length()==0 || txtRubro.getText().length()==0){
                JOptionPane.showMessageDialog(null, "Debe completar los datos del formulario para continuar.","Error",JOptionPane.ERROR_MESSAGE);
                return false;
            }else{
                if (clave1.length == clave2.length){
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Ingrese el rut o parte del nombre del proveedor para realizar una búsqueda. El sistema buscará todos los registros coincidentes para desplegarlos.");
        ...
        jTablaProveedores.getTableHeader().setReorderingAllowed(false);
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        ...
        if (ValidarCampos()){
            ...
            if (confirmar == JOptionPane.YES_NO_OPTION){
                ...
                p.setRubro(txtRubro.getText());
                if (idProveedor == 0){
                    ControladorProveedor ctrP = new ControladorProveedor();
                    if (ctrP.AgregarProveedor(p)){
                        Funciones f = new Funciones();
                        ...
                        a.setUsuario_id(0);
                        a.setProveedor_id(ctrA.ObtenerIDProveedor(txtCorreo.getText()));

                        if (ctrA.AgregarUsuarioProveedor(a)){
                            JOptionPane.showMessageDialog(null, "Proveedor generado correctamente.","Crear proveedor",JOptionPane.INFORMATION_MESSAGE);
                            Limpiar();
                            CargarDatos();
                            CambiarEstado(false);
                        ...
                        JOptionPane.showMessageDialog(null, "Ocurrio un error al generar el proveedor.","Crear proveedor",JOptionPane.ERROR_MESSAGE);
                    }         
                }else{
                    ControladorProveedor ctrP = new ControladorProveedor();
                    if (ctrP.ModificarProveedor(p)){
                        Funciones f = new Funciones();
                        ControladorAcceso ctrA = new ControladorAcceso();
                        Acceso a = new Acceso();
                        a.setUsuario(txtCorreo.getText());
                        ...
                        a.setProveedor_id(idProveedor);
                        if (ctrA.ModificarUsuarioProveedor(a)){
                            JOptionPane.showMessageDialog(null, "Proveedor actualizado correctamente.","Modificar proveedor",JOptionPane.INFORMATION_MESSAGE);
                            Limpiar();
                            CargarDatos();
                            CambiarEstado(false);
                            idProveedor = 0;
                        ...
                        JOptionPane.showMessageDialog(null, "Ocurrio un error al modificar al proveedor.","Error",JOptionPane.ERROR_MESSAGE);
                    }
                }
            }else{
                txtRut.requestFocus();
    ...
    private void txtRutFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtRutFocusLost
        ...
        if (txtRut.getText().length() != 0){
            ...
            ControladorProveedor ctrP = new ControladorProveedor();
            if (ctrP.ExisteRut(txtRut.getText())){
    ...
    private void txtCorreoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtCorreoFocusLost
        ...
        if (txtCorreo.getText().length() != 0){
            ...
            ControladorProveedor ctrP = new ControladorProveedor();
            if (ctrP.ExisteCorreo(txtCorreo.getText())){
    ...
    private void jTablaProveedoresMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTablaProveedoresMouseClicked
        ...
        if (evt.getClickCount() == 2){
            int fila = jTablaProveedores.getSelectedRow();
            if (fila != -1){
                DefaultTableModel modelo = (DefaultTableModel)jTablaProveedores.getModel();
                idProveedor = Integer.parseInt(modelo.getValueAt(fila, 0).toString());
                ...
                txtRubro.setText(modelo.getValueAt(fila, 6).toString());
                lbCorreo.setText(txtCorreo.getText());
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        // TODO add your handling code here:
        int confirmar = JOptionPane.showConfirmDialog(null, "¿Esta seguro de eliminar a este proveedor?","Eliminar proveedor",JOptionPane.YES_NO_OPTION);
        if (confirmar == JOptionPane.YES_NO_OPTION){
            ControladorProveedor ctrP = new ControladorProveedor();
            ControladorAcceso ctrA = new ControladorAcceso();
            if (ctrA.EliminarUsuarioProveedor(idProveedor)){
                if (ctrP.EliminarProveedor(idProveedor)){
                    JOptionPane.showMessageDialog(null, "Proveedor eliminado satisfactoriamente del sistema.","Proveedor eliminado",JOptionPane.INFORMATION_MESSAGE);
                    idProveedor = 0;
                    Limpiar();
                    CambiarEstado(false);
                    btnNuevo.setEnabled(true);
                    ...
                    JOptionPane.showMessageDialog(null, "Ocurrio un error al eliminar al proveedor del sistema.","Error",JOptionPane.ERROR_MESSAGE);
                }
            }else{
                JOptionPane.showMessageDialog(null, "Ocurrio un error al eliminar la cuenta de acceso del proveedor.","Error",JOptionPane.ERROR_MESSAGE);
            }
        }
