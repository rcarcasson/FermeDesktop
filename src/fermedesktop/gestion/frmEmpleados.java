...
public class frmEmpleados extends javax.swing.JInternalFrame {
    ...
    private void CargarDatos(){
        ...
        ArrayList<Usuario> lista = ctrlE.ObtenerEmpleados(txtCadena.getText());
        for (int i=0; i<lista.size();i++){
            String tipo = "";
            if (lista.get(i).getTipo_usuario()=='E'){
                tipo = "Empleado";
            }else if (lista.get(i).getTipo_usuario()=='V'){
                tipo ="Vendedor";
            }
            modelo.addRow(new Object[]{lista.get(i).getId(),lista.get(i).getRut(),lista.get(i).getNombre(),
            lista.get(i).getDireccion(),lista.get(i).getTelefono(),lista.get(i).getMail(),lista.get(i).getCargo(),tipo});
        }
        jTablaEmpleados.setModel(modelo);
        jTablaEmpleados.getTableHeader().getColumnModel().getColumn(0).setMaxWidth(0);
        jTablaEmpleados.getTableHeader().getColumnModel().getColumn(0).setMinWidth(0);
        
        jTablaEmpleados.getColumn("ID").setMaxWidth(0);
        jTablaEmpleados.getColumn("ID").setMinWidth(0);
        jTablaEmpleados.getColumn("ID").setPreferredWidth(0);        
    }
    ...
    private String[] getColumnas(){
        String[] columnas = new String[]{"ID","RUT","Nombre","Dirección","Teléfono","Mail","Cargo","Tipo"};
        return columnas;
    }
    ...
    private void Limpiar(){
        ...
        lbCorreo.setText("");
        lbClave.setText("");
        opEmpleado.setSelected(false);
        opVendedor.setSelected(false);
    }
    ...
    private boolean ValidarCampos(){
        ...
        if (idEmpleado==0){
            if (txtRut.getText().length()==0 || txtNombre.getText().length()==0 || txtDireccion.getText().length()==0 ||
                    txtTelefono.getText().length()==0 || txtCorreo.getText().length()==0 || txtCargo.getText().length()==0 || cl1.length()==0 || cl2.length()==0 || (!opVendedor.isSelected() && !opEmpleado.isSelected())){
            ...
            if (txtRut.getText().length()==0 || txtNombre.getText().length()==0 || txtDireccion.getText().length()==0 ||
                    txtTelefono.getText().length()==0 || txtCorreo.getText().length()==0 || txtCargo.getText().length()==0 || (!opVendedor.isSelected() && !opEmpleado.isSelected())){
                JOptionPane.showMessageDialog(null, "Debe completar los datos del formulario para continuar.","Error",JOptionPane.ERROR_MESSAGE);
                return false;
            }else{
                if (clave1.length == clave2.length){
    ...
    private void CambiarEstado(boolean estado){
        ...
        opVendedor.setEnabled(estado);
        opEmpleado.setEnabled(estado);
    }
    /**
    ...
    private void initComponents() {
        ...
        opEmpleado = new javax.swing.JRadioButton();
        opVendedor = new javax.swing.JRadioButton();
        ...
        jLabel1.setText("Ingrese el rut o parte del nombre del empleado para realizar una búsqueda. El sistema buscará todos los registros coincidentes para desplegarlos.");
        ...
        jTablaEmpleados.getTableHeader().setReorderingAllowed(false);
        ...
        jTablaEmpleados.getColumnModel().getSelectionModel().setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        ...
        opGrupoTipo.add(opEmpleado);
        opEmpleado.setText("Empleado");

        opGrupoTipo.add(opVendedor);
        opVendedor.setText("Vendedor");
                    ...
                    .addGroup(jPanelGestionLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                        ...
                        .addGroup(jPanelGestionLayout.createSequentialGroup()
                            ...
                            .addGroup(jPanelGestionLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                .addComponent(txtDireccion, javax.swing.GroupLayout.DEFAULT_SIZE, 563, Short.MAX_VALUE)
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        // TODO add your handling code here:
        if (ValidarCampos()){
            int confirmar = JOptionPane.showConfirmDialog(null, "¿Desea guardar los datos del proveedor?","Guardar",JOptionPane.YES_NO_OPTION);
            if (confirmar == JOptionPane.YES_NO_OPTION){
                ...
                if (opVendedor.isSelected()){
                    u.setTipo_usuario('V');
                }else if(opEmpleado.isSelected()){
                    u.setTipo_usuario('E');
                }
                if (idEmpleado == 0){
                    ControladorEmpleado ctrE = new ControladorEmpleado();
                    if (ctrE.AgregarEmpleado(u)){
                        Funciones f = new Funciones();
                        ...
                        if (ctrA.AgregarUsuario(a)){
                            JOptionPane.showMessageDialog(null, "Empleado generado correctamente.","Crear empleado",JOptionPane.INFORMATION_MESSAGE);
                            Limpiar();
                            CargarDatos();
                            CambiarEstado(false);
                        ...
                        JOptionPane.showMessageDialog(null, "Ocurrio un error al generar el empleado.","Crear empleado",JOptionPane.ERROR_MESSAGE);
                    }         
                }else{
                    ControladorEmpleado ctrE = new ControladorEmpleado();
                    if (ctrE.ModificarEmpleado(u)){
                        Funciones f = new Funciones();
                        ControladorAcceso ctrA = new ControladorAcceso();
                        Acceso a = new Acceso();
                        a.setUsuario(txtCorreo.getText());
                        ...
                        a.setUsuario_id(idEmpleado);
                        if (ctrA.ModificarUsuario(a)){
                            JOptionPane.showMessageDialog(null, "Empleado actualizado correctamente.","Modificar empleado",JOptionPane.INFORMATION_MESSAGE);
                            Limpiar();
                            CargarDatos();
                            CambiarEstado(false);
                            idEmpleado = 0;
                        ...
                        JOptionPane.showMessageDialog(null, "Ocurrio un error al modificar al empleado.","Error",JOptionPane.ERROR_MESSAGE);
                    }
                }
            }else{
                txtRut.requestFocus();
    ...
    private void txtCorreoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtCorreoFocusLost
        ...
        if (txtCorreo.getText().length() != 0){
            ...
            if (ctrU.ExisteCorreo(txtCorreo.getText())){
                JOptionPane.showMessageDialog(null, "Ya existe un usuario registrado con este correo.","Mail existente",JOptionPane.INFORMATION_MESSAGE);
                txtCorreo.requestFocus();
                txtCorreo.selectAll();
            }
        }
    ...
    private void jTablaEmpleadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTablaEmpleadosMouseClicked
        ...
        if (evt.getClickCount() == 2){
            ...
            if (fila != -1){
                DefaultTableModel modelo = (DefaultTableModel)jTablaEmpleados.getModel();
                idEmpleado = Integer.parseInt(modelo.getValueAt(fila, 0).toString());
                ...
                String tipo = modelo.getValueAt(fila, 7).toString();
                if (tipo.equals("Vendedor")){
                    opVendedor.setSelected(true);
                }else if (tipo.equals("Empleado")){
                    opEmpleado.setSelected(true);
                }
                lbCorreo.setText(txtCorreo.getText());
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        // TODO add your handling code here:
        int confirmar = JOptionPane.showConfirmDialog(null, "¿Esta seguro de eliminar a este empleado?","Eliminar empleado",JOptionPane.YES_NO_OPTION);
        if (confirmar == JOptionPane.YES_NO_OPTION){
            ControladorEmpleado ctrE = new ControladorEmpleado();
            ControladorAcceso ctrA = new ControladorAcceso();
            if (ctrA.EliminarUsuario(idEmpleado)){
                if (ctrE.EliminarEmpleado(idEmpleado)){
                    JOptionPane.showMessageDialog(null, "Empleado eliminado satisfactoriamente del sistema.","Empleado eliminado",JOptionPane.INFORMATION_MESSAGE);
                    idEmpleado = 0;
                    Limpiar();
                    CambiarEstado(false);
                    btnNuevo.setEnabled(true);
                    ...
                    JOptionPane.showMessageDialog(null, "Ocurrio un error al eliminar al empleado del sistema.","Error",JOptionPane.ERROR_MESSAGE);
                }
            }else{
                JOptionPane.showMessageDialog(null, "Ocurrio un error al eliminar la cuenta de acceso del empleado.","Error",JOptionPane.ERROR_MESSAGE);
            }
        }
    ...
    private javax.swing.JRadioButton opEmpleado;
    private javax.swing.ButtonGroup opGrupoTipo;
    private javax.swing.JRadioButton opVendedor;
